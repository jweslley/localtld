...
void
localtld_fill_hostent(const char *name,
                  int af,
...
enum nss_status
_nss_localtld_gethostbyname2_r(const char *name,
                           int af,
                           struct hostent *result,
                           char *buffer,
                           size_t buflen,
    ...
    while (domain != NULL) {
        ...
        if (name_ext != NULL) {
            ...
            if (strcasecmp(name_ext, ext) == 0) {
                ...
                localtld_fill_hostent(name, af, result);
                free(ext);
                break;
            }
            free(ext);
...
enum nss_status
_nss_localtld_gethostbyname_r(const char *name,
                          struct hostent *result,
                          char *buffer,
                          size_t buflen,
                          int *errnop,
                          int *h_errnop)
{
    return _nss_localtld_gethostbyname2_r(name, AF_INET, result, buffer, buflen, errnop, h_errnop);
}
